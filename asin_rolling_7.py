...
def get_asin_data():
    print("Please upload ASIN performance csv")
    ...
    data = pd.read_csv(filename, parse_dates=['formatted_date'])
    ...
    def preprocess(data):
        ...
        def add_organic_data(data):
            data = (data
                        ...
                        .sort_index(ascending=True) # Ensure the data is in ascending order
    ...
    # 7 day heatmap
    def heatmap_7(data):
        heatmap = data.fillna(0).iloc[::-1].iloc[:45, -6:].fillna(0).style.apply(apply_color_scale)
        return heatmap
    ...
    # Generate heatmap and plots
    heatmap = heatmap_7(data)
    fig = plot_metrics(data)

    return heatmap, fig

# Example usage:
# heatmap, fig = get_asin_data()
